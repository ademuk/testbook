...
const mergeComponents = (
  ...
  baseComponents.map(
    (bc) => components.find((c) => c.exportName === bc.exportName) || bc
  );

const mergeModules = (
  modules: ModuleDefinition[],
  tests: ModuleDefinition[]
) => {
...
const splitIntoErrorNoError = (modules: ModuleDefinition[]) => {
  const modulesWithOnlyErrors = modules.filter((m) =>
    m.components.every((c) => c.error)
  );

  return [
    modulesWithOnlyErrors,
    modules.filter(
      ({ file }) => !modulesWithOnlyErrors.find((f) => f.file === file)
    ),
  ];
};
...
export default function Components() {
  ...
  const [testsStatus, setTestsStatus] = useState<LoadingStatus>(
    LoadingStatus.pending
  );
  const [modules, setModules] = useState<ModuleDefinition[]>([]);
  const [modulesStatus, setModulesStatus] = useState<LoadingStatus>(
    LoadingStatus.pending
  );
  ...
  return (
    <div className="p-3">
      <h1 className="p-6 text-gray-700 text-2xl font-semibold py-2">
        Your tests
      </h1>
      {tests.map((module) => (
        <Module
      ...
      {modulesStatus === LoadingStatus.loading && (
        <div className="p-6">
          <LoadingIndicator>Looking for components...</LoadingIndicator>
        </div>
      )}
      {!!modulesWithOnlyErrors.length && (
        <div className="p-6 block text-gray-700 text-2xl font-semibold py-2">
          Other modules which had errors
        </div>
      )}
      {modulesWithOnlyErrors.map((m) => (
