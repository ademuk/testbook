...
const EditRenderWrapperModal: React.FC<EditStepProps> = ({
  step,
  onClose,
  onUpdateStep,
  file,
  ...
  const [
    wrapperOptionsStatus,
    setWrapperOptionsStatus,
  ] = useState<WrapperOptionsStatus>();
  ...
  const handleWrapperSelect = (file: string, exportName: string) => {
    if (
      file === step.definition.wrapper?.file &&
      exportName === step.definition.wrapper?.exportName
    ) {
      return onUpdateStep({
        ...step,
        definition: {
  ...
  return (
    <Modal onClose={onClose}>
      <ModalBody>
        ...
        <div className="mt-2">
          {wrapperOptionsStatus === WrapperOptionsStatus.loading && (
            <LoadingIndicator>Looking for wrapper options...</LoadingIndicator>
          )}
          {wrapperOptionsStatus === WrapperOptionsStatus.error && (
            <div className="text-red-700">
              There was an issue fetching wrapper options
            </div>
          )}
          {wrapperOptions
            ? wrapperOptions.map(({ file, exportName }) => (
                <div className="mb-4" key={file + exportName}>
                  <button
                    ...
                    className={`${
                      selectedWrapper &&
                      selectedWrapper.file === file &&
                      selectedWrapper.exportName === exportName
