...
const defaultStepRenderer = (step: StepDefinition) =>
  Object.entries(step)
    .map(
      ([key, val]) =>
...
const STEP_LABELS: { [key: string]: (step: StepDefinition) => string } = {
  ...
  assertion: ({ definition: { type, target } }: StepDefinition) =>
    type === "text"
      ...
      : `Assert ${type} ${target.name} ${renderMockCallArgsLabel(
          target.args
        )} called`,
  mock: ({ definition: { name, args } }: StepDefinition) =>
    `Mock ${name} ${renderMockCallArgsLabel(args)}`,
};
...
const Step = ({
  ...
  return (
    ...
    >
      ...
      <div className="block">
        {active && ["render", "mock"].includes(type) && (
          <button
            onClick={onEdit}
